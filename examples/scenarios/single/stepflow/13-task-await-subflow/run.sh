#!/bin/bash
# ============================================================================
# 13-task-await-subflow: Task awaits a child workflow completion
# ============================================================================
...
header "Stepflow: 13-task-await-subflow"
...
CHILD_WORKFLOW=$(random_name "starter-child-flow")
PARENT_WORKFLOW=$(random_name "starter-parent-flow")

CHILD_DSL="$SCRIPT_DIR/workflow-child.json"
PARENT_TEMPLATE="$SCRIPT_DIR/workflow-parent.json"
PARENT_DSL="$(mktemp "${TMPDIR:-/tmp}/stepflow-await-13-XXXX.json")"
...
if ! $CLI wf create "$CHILD_WORKFLOW" --dsl "$CHILD_DSL" --description "Starter: 13-task-await-subflow child"; then
...
if ! $CLI wf create "$PARENT_WORKFLOW" --dsl "$PARENT_DSL" --description "Starter: 13-task-await-subflow parent"; then
...
echo ""
success "13-task-await-subflow completed successfully"
