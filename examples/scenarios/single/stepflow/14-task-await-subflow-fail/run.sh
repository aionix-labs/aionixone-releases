#!/bin/bash
# ============================================================================
# 14-task-await-subflow-fail: Await a failing child workflow
# ============================================================================
...
header "Stepflow: 14-task-await-subflow-fail"
...
CHILD_WORKFLOW=$(random_name "starter-child-fail")
PARENT_WORKFLOW=$(random_name "starter-parent-fail")

CHILD_DSL="$SCRIPT_DIR/workflow-child.json"
PARENT_TEMPLATE="$SCRIPT_DIR/workflow-parent.json"
PARENT_DSL="$(mktemp "${TMPDIR:-/tmp}/stepflow-await-14-XXXX.json")"
...
if ! $CLI wf create "$CHILD_WORKFLOW" --dsl "$CHILD_DSL" --description "Starter: 14-task-await-subflow-fail child"; then
...
if ! $CLI wf create "$PARENT_WORKFLOW" --dsl "$PARENT_DSL" --description "Starter: 14-task-await-subflow-fail parent"; then
...
echo ""
success "14-task-await-subflow-fail completed successfully"
