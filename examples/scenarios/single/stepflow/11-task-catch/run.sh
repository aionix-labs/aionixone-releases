#!/bin/bash
# ============================================================================
# 11-task-catch: Branching execution on task failure using catch
# ============================================================================
...
WORKFLOW_NAME=$(random_name "starter-task-catch")
DSL_TEMPLATE="$SCRIPT_DIR/workflow.json"
DSL_FILE="$(mktemp "${TMPDIR:-/tmp}/stepflow-bun-11-XXXX.json")"
...
if ! $CLI wf create "$WORKFLOW_NAME" --dsl "$DSL_FILE" --description "Starter: 11-task-catch"; then
    fail "Failed to create workflow"
fi
success "Workflow created"

# Step 2: Run workflow
info "Running workflow (task will fail, catch will handle it)..."
info "  - RiskyTask executes Bun fail operation (will fail)"
info "  - catch block routes to HandleError step"
...
# Step 3: Wait for completion (expecting succeeded because catch handles the error)
...
while [ $WAITED -lt $timeout ]; do
    ...
    case "$STATUS" in
        succeeded)
            success "Workflow completed: $STATUS (error was caught)"
            break
            ;;
        failed)
            fail "Workflow failed (catch did not work as expected)"
            ;;
        cancelled|timeout)
...
echo ""
success "11-task-catch completed successfully"
