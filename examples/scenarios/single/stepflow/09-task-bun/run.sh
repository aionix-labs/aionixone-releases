#!/bin/bash
# ============================================================================
# 09-task-bun: Execute a Bun function as a workflow step
# ============================================================================
...
WORKFLOW_NAME=$(random_name "starter-task-bun")
DSL_TEMPLATE="$SCRIPT_DIR/workflow.json"
DSL_FILE="$(mktemp "${TMPDIR:-/tmp}/stepflow-bun-09-XXXX.json")"
...
if ! $CLI wf create "$WORKFLOW_NAME" --dsl "$DSL_FILE" --description "Starter: 09-task-bun"; then
    fail "Failed to create workflow"
fi
success "Workflow created"

# Step 2: Run workflow
info "Running workflow (executes: Bun greet operation)..."
...
echo ""
success "09-task-bun completed successfully"
