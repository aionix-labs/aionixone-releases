#!/bin/bash
# ============================================================================
# 06-router: Conditional routing with router step
# ============================================================================
...
WORKFLOW_NAME=$(random_name "starter-router")
...
if ! $CLI wf create "$WORKFLOW_NAME" --dsl "$DSL_FILE" --description "Starter: 06-router"; then
    fail "Failed to create workflow"
fi
success "Workflow created"

# Step 2: Run workflow with value > 10 (should go to HighValue)
info "Running workflow with value=15 (should route to HighValue)..."
RUN_OUTPUT=$($CLI wf run "$WORKFLOW_NAME" --data '{"value": 15}' 2>&1)
...
echo ""
success "06-router completed successfully"
